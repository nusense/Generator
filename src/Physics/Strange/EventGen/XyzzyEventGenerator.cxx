...
#include "Physics/Strange/EventGen/XyzzyEventGenerator.h"
...
XyzzyEventGenerator::XyzzyEventGenerator() :
    KineGeneratorWithCache("genie::XyzzyEventGenerator")
{

}
//___________________________________________________________________________
XyzzyEventGenerator::XyzzyEventGenerator(string config) :
    KineGeneratorWithCache("genie::XyzzyEventGenerator", config)
{
...
//___________________________________________________________________________
void XyzzyEventGenerator::ProcessEventRecord(GHepRecord * evrec) const
{
    LOG("XyzzyEvent", pDEBUG) << "Generating Xyzzy event kinematics...";
    LOG("XyzzyEvent", pNOTICE) << "Generating Xyzzy event kinematics...";
...
/*
    ...
    if(interaction->InitState().Tgt().IsNucleus()) {
        ...
        if(! evrec->HitNucleon()) {
            LOG("XyzzyEvent", pFATAL) << "No hit nucleon was set";
            gAbortingInErr = true;
            exit(1);
        }
    } // is nuclear target
    ...
    while (1) {
        ...
        iter++;
        LOG("XyzzyEvent", pINFO) << "Attempt #: " << iter;
        if(iter > kRjMaxIterations) {
            LOG("XyzzyEvent", pWARN)
        ...
        // Pick a direction
        // NOTE: In the kPSXyzzyEvGen phase space used by this generator,
        ...
        // Set the "bind_nucleon" flag to false in this call to ComputeFullXyzzyPXSec
        // since we've already done that above
        LOG("XyzzyEvent", pDEBUG) << "cth0 = " << costheta << ", phi0 = " << phi;
        double xsec = genie::utils::ComputeFullXyzzyPXSec(interaction, fNuclModel,
        ...
        if(accept) {
            ...
            LOG("XyzzyEvent", pINFO) << "*Selected* Q^2 = " << gQ2 << " GeV^2";
            ...
            LOG("XyzzyKinematics", pNOTICE) << "E = " << E << ", M = "<< M;

            // The hadronic inv. mass is equal to the recoil nucleon on-shell mass.
            // For Xyzzy/Charm events it is set to be equal to the on-shell mass of
            ...
            LOG("XyzzyEvent", pNOTICE) << "Selected: W = "<< gW;
            ...
            evrec->SetDiffXSec(xsec, kPSXyzzyEvGen);
            ...
            LOG("XyzzyEvent",pNOTICE) << "pn: " << p4ptr.X() << ", "
            ...
            LOG("XyzzyEvent", pDEBUG) << "Reject current throw...";
        }

    } // iterations - while(1) loop
*/

    LOG("XyzzyEvent", pINFO) << "Done generating Xyzzy event kinematics!";
    LOG("XyzzyEvent", pNOTICE) << "Done generating Xyzzy event kinematics!";
    LOG("XyzzyEvent", pNOTICE) << '\n' << *evrec;
}
//___________________________________________________________________________
void XyzzyEventGenerator::AddTargetNucleusRemnant(GHepRecord * evrec) const
{
    // add the remnant nuclear target at the GHEP record

  /*
    LOG("XyzzyEvent", pINFO) << "Adding final state nucleus";
    ...
    LOG("XyzzyEvent", pINFO) << "Done";
    LOG("XyzzyEvent", pINFO) << *evrec;
  */

}
//___________________________________________________________________________
void XyzzyEventGenerator::Configure(const Registry & config)
{
    Algorithm::Configure(config);
    this->LoadConfig();
}
//____________________________________________________________________________
void XyzzyEventGenerator::Configure(string config)
{
    Algorithm::Configure(config);
    this->LoadConfig();
}
//____________________________________________________________________________
void XyzzyEventGenerator::LoadConfig(void)
{
...
/*
    ...
    fHitNucleonBindingMode = genie::utils::StringToXyzzyBindingMode( binding_mode );
...
//____________________________________________________________________________
double XyzzyEventGenerator::ComputeMaxXSec(const Interaction * in) const
{
    ...
    LOG("XyzzyEvent", pINFO) << "Computing maximum cross section to throw against";
...
/*
...

    {
        ...
        for (int ilayer = 0 ; ilayer < max_n_layers ; ilayer++) {
          ...
          for (int itheta = 0; itheta < N_theta; itheta++){
              ...
              for (int iphi = 0; iphi < N_phi; iphi++) { // Scan around phi
                  ...
                  double xs = genie::utils::ComputeFullXyzzyPXSec(interaction,
        ...
        if (this_nuc_xsec_max > xsec_max) {
            ...
            LOG("XyzzyEvent", pINFO) << "best estimate for xsec_max = " << xsec_max;
        }
...
#ifdef __GENIE_LOW_LEVEL_MESG_ENABLED__
    SLOG("XyzzyEvent", pDEBUG) << interaction->AsString();
    SLOG("XyzzyEvent", pDEBUG) << "Max xsec in phase space = " << max_xsec;
    SLOG("XyzzyEvent", pDEBUG) << "Computed using alg = " << *fXSecModel;
#endif

    LOG("XyzzyEvent", pINFO) << "Computed maximum cross section to throw against - value is " << xsec_max;
*/
    return xsec_max;
}
//____________________________________________________________________________
